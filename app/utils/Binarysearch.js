...
export default class Binarysearch {
	...
	static async findMaxOutputIter(getAmountsOutOfRoute, routes, bid) {
	  // Initial allocation
	  const allocation = routes.map(r => 0)
	  let inputs = routes.map(r => 0)
	  let outputs = routes.map(r => 0)

	  let i = 0;
	  let totalIncrement = 0
	  while(i < maxSteps && totalIncrement < stepPrecision) {
	  	const stepIncrement = Math.max(1, Math.floor((stepPrecision - totalIncrement)/(2*routes.length)) )
	    let candidateResults = await Binarysearch.objectiveIter(getAmountsOutOfRoute, allocation.map(a => a + stepIncrement), bid, routes)
