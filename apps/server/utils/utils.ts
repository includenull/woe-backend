...
export const isPairReverted = (pair) => {
  ...
  for (const rtoken of revert_tokens)
    if (
      pair.token1.symbol.ticker === rtoken.ticker &&
      rtoken.contract === pair.token1.contract
    )
      return true;
    else if (
      pair.token0.symbol.ticker === rtoken.ticker &&
      rtoken.contract === pair.token0.contract
    )
      return false;
...
export const precise = (price, precision) => {
  ...
  const rounded = Number(
    Math.floor(Number(price + "e" + precision)) +
      "e-" +
      (precision - additionalExposant),
  );
...
export const makeDateForSmartcontract = (timestamp) => {
  ...
  const date =
    d.getUTCFullYear() +
    "-" +
    forceStrTwoDigit(1 + d.getUTCMonth()) +
    "-" +
    forceStrTwoDigit(d.getUTCDate());
  const hours =
    forceStrTwoDigit(d.getUTCHours()) +
    ":" +
    forceStrTwoDigit(d.getUTCMinutes()) +
    ":" +
    forceStrTwoDigit(d.getUTCSeconds());

  return (
    date + "T" + hours + "." + (d.getUTCMilliseconds() === 0 ? "000" : "500")
  );
};
